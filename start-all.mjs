 ...
 *
 * 运行时：完全使用 process.execPath（启动本脚本的 node 自身）——项目用便携 node
 * （C:\Users\Friend\bin\node，不在系统 PATH），不依赖 PATH 里的 node/npm.cmd。
 *
...
run('engine', resolve(root, 'engine'), process.execPath, ['main.ts'])
run('ui', resolve(root, 'UI'), process.execPath, ['node_modules/vite/bin/vite.js'])
