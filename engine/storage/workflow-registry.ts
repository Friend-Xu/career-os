...
export const CAREER_DIRECTION_STAGES: StageSpec[] = [
  {
    ...
    task: {
      ...
      instructions: [
        ...
        '结构化载荷（引擎据此登记投影，缺载荷 = 该候选无法登记）：教育类目必须附加第四段：候选标记：教育｜{内容}｜{来源}｜学校=…；专业=…；学历=…；起=…；止=…（学历只能是：高中、大专、本科、硕士、博士；年份为数字如 2019）。工作经历类目必须附加第四段：候选标记：经历｜{内容}｜{来源}｜公司=…；岗位=…；起=…；止=…（起止 YYYY.MM 如 2023.07；仍在职写止=至今）。技能类目必须附加第四段：候选标记：技能｜{内容}｜{来源}｜技能=…；级别=…；场景=…（级别只能是：熟练、胜任、掌握、入门；场景=使用场景简述）。约束类目可选附加第四段：候选标记：约束｜{内容}｜{来源}｜薪资=…；城市=…；现居=…。项目经历/Gap/比赛类经历候选不附加第四段；其余类目省略第四段',
        '不要自行写 persons/{person_id}/extraction/candidates.md 等候选文件——候选登记由会话通道落盘（前端/引擎 appendCandidates）；缺少候选标记行 = 该条信息不会被系统收集',
        '示例：候选标记：教育｜机械设计制造及其自动化本科｜简历｜学校=某大学；专业=机械设计制造及其自动化；学历=本科；起=2015；止=2019',
        '候选必须带来源（用户描述/简历），禁止编造',
      ...
      outputBudget: 8192, // 真机实测：候选列表 + 候选标记行（含结构化载荷）为一轮输出；4096 会截断（消息尾部中断、标记行未产出 → 0 候选 → 阶段失败）
    },
  },
  {
