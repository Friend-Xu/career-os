...
const VALIDATION_STYLE: Record<string, { color: string; icon: string }> = {
  valid: { color: RISK_COLOR.low, icon: '✓' },
  warning: { color: RISK_COLOR.medium, icon: '△' },
  invalid: { color: RISK_COLOR.high, icon: '✗' },
}
...
export function ResumeStudio() {
  ...
  if (resumeVersions.length === 0) {
    return (
      <Box sx={{ p: 3, borderRadius: '10px', border: `1px solid ${alpha(COLORS.border, 0.8)}`, boxShadow: COLORS.cardShadow, bgcolor: COLORS.bg }}>
        ...
        <Typography sx={{ fontSize: 12, color: COLORS.textMuted }}>版本系统：AI 产出 Draft Manifest → 引擎组装 → 此处可见版本进化</Typography>
      </Box>
    )
  }
  ...
  const doClone = async (id: string): Promise<void> => {
    try {
      ...
      push('success', '已创建变体（新 draft，parent 已记录）')
    } catch {
      push('warning', '克隆失败')
    }
  }
  ...
  const doReview = async (id: string): Promise<void> => {
    try {
      ...
      push('success', '已提交确认（draft → review）')
    } catch (e) {
  ...
  const doExport = async (id: string): Promise<void> => {
    try {
      ...
      push('success', `已导出 ${result.fileName}（status=exported，绑定 ExportRecord）`)
    } catch (e) {
  ...
  const doAccept = async (id: string): Promise<void> => {
    try {
      ...
      push('success', `已接受提案 → 新版本 ${doc.id}（lineage parent 已记录，源版本未动）`)
    } catch (e) {
  ...
  const askProposal = (): void => {
    startAnalysis(
      `请基于当前选中简历版本「${selected?.id ?? '未选中'}」提出改进建议（Proposal Layer）：读取 ai/context 与 resumes/get 获取源版本与岗位期望，按 references/proposal-writer.md 契约写 proposals/*.md 提案文件——只写提案，绝不修改 resumes/documents/。`,
    )
  ...
  return (
    <Box>
                  ...
                  <Stack spacing={0.25}>
                    {s.bullets.map((b, bi) => (
                      ...
                      >
                        {b.sentence}
                        <Typography component="span" sx={{ fontSize: 11, color: COLORS.textMuted, ml: 0.75 }}>
                          · claim {b.claimId.slice(-4)}
                        </Typography>
                      </Typography>
                    ))}
                    {(s.assetRefs ?? []).map((a, ai) => (
                      <Typography key={`a${ai}`} sx={{ fontSize: 12.5, color: COLORS.textSecondary, lineHeight: 1.7 }}>
                        {a} <Typography component="span" sx={{ fontSize: 11, color: COLORS.textMuted }}>· asset</Typography>
                      </Typography>
                    ))}
                  </Stack>
                </Box>
          ...
          <Box sx={{ p: 2, borderRadius: '10px', border: `1px solid ${alpha(COLORS.border, 0.8)}`, boxShadow: COLORS.cardShadow, bgcolor: COLORS.bg }}>
            ...
            {selectedClaim ? (
              (() => {
                ...
                if (!c) return <Typography sx={{ fontSize: 12, color: COLORS.textMuted }}>Claim 未找到：{selectedClaim}</Typography>
                return (
                  <Stack spacing={1}>
                    <Box>
                      <Typography sx={{ fontSize: 11, color: COLORS.textMuted, mb: 0.25 }}>Claim · {c.type === 'fact' ? '事实' : '归纳'}</Typography>
                      <Typography sx={{ fontSize: 12.5, color: COLORS.text, lineHeight: 1.6 }}>{c.statement}</Typography>
                    </Box>
                    <Box>
                      <Typography sx={{ fontSize: 11, color: COLORS.textMuted, mb: 0.25 }}>可消费性（canUseClaim）</Typography>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        startAnalysis(`请解释简历中这条 bullet 的 provenance：Claim「${c.statement}」为什么被选中、它对应的岗位期望与证据是什么、是否有更好的表达方式（按 sentence-generator 契约）`)
                        push('info', '已预置「Provenance 解释」上下文')
                      }}
                      sx={{ fontSize: 11.5 }}
                    >
                      Ask AI
              ...
              <Typography sx={{ fontSize: 12, color: COLORS.textMuted }}>点击左侧 bullet 查看其 provenance</Typography>
            )}
          </Box>
        </Box>
      )}
          ...
          {selected?.validation ? (
            <Stack spacing={0.5}>
              <Typography sx={{ fontSize: 12.5, color: VALIDATION_STYLE[selected.validation.status].color }}>
                {VALIDATION_STYLE[selected.validation.status].icon} {selected.validation.status.toUpperCase()}
              </Typography>
              ...
              {selected.validation.issues.length > 0 && (
                <Button
                  ...
                  startIcon={<PsychologyIcon sx={{ fontSize: 14 }} />}
                  onClick={() => {
                    startAnalysis(`请解释简历版本「${selected.id}」的 validation 问题：${selected.validation!.issues.map((i) => `${i.code}（${i.message}）`).join('、')}——为什么出现、如何修正（修正走 Draft 流程，不直接改版本）`)
                    push('info', '已预置「Validation 解释」上下文')
                  }}
      ...
      <Dialog open={diffOpen} onClose={() => setDiffOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontSize: 14, fontWeight: 600, pb: 1 }}>版本对比（identity diff）</DialogTitle>
        <DialogContent>
          <Stack spacing={1.5}>
            ...
            {diffResult && (
              <Box>
                {diffResult.added.length > 0 && (
                  <Box sx={{ mb: 1 }}>
                    <Typography sx={{ fontSize: 12, fontWeight: 600, color: RISK_COLOR.low, mb: 0.25 }}>Added（+{diffResult.added.length}）</Typography>
                    {diffResult.added.map((i, idx) => (
                      <Typography key={idx} sx={{ fontSize: 12, color: COLORS.text, lineHeight: 1.6 }}>
                        ✓ {i.sentence} <Typography component="span" sx={{ fontSize: 11, color: COLORS.textMuted }}>· claim {i.claimId.slice(-4)}</Typography>
                      </Typography>
                    ))}
                  </Box>
                )}
                {diffResult.removed.length > 0 && (
                  <Box sx={{ mb: 1 }}>
                    <Typography sx={{ fontSize: 12, fontWeight: 600, color: RISK_COLOR.high, mb: 0.25 }}>Removed（−{diffResult.removed.length}）</Typography>
                ...
                {diffResult.unchanged.length > 0 && (
                  <Typography sx={{ fontSize: 11.5, color: COLORS.textMuted }}>Unchanged（{diffResult.unchanged.length} 条一致）</Typography>
                )}
              </Box>
            )}
          </Stack>
