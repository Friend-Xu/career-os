...
| 公司名 | **canonical reference，不是自由文本**——写入前先查 companies/ 档案（对齐 Company Artifact Admission Step 0），岗位标题公司名写**解析后的档案名**；JD 简称/工商全称若与档案名不一致，禁止直接写入（alias 由公司档案声明，岗位清单不发明新名称）。实例：「Company-A 医疗」是档案名，「Company-A 医疗科技股份有限公司」是 alias——岗位写「管理培训生（Company-A 医疗）」 |
