...
>
> **修订记录 v0.2（2026-08-14，匹配度引擎化实测审计驱动）**：真实 JD 分析审计发现三类
> 提取误判（管培生类 JD：培养内容提取为 hard 要求；工程类 JD：「A或B等至少一款」OR
> 语义拆成并列要求；「客户需求转化」行为词入 hard）——假要求产生假缺口，匹配度继承错误。
> 增补三条提取规则（§2.1 要求源边界 / §2.2 OR 语义 / §2.3 行为词分类）与门槛自查
> （§2.4）。存量旧格式分析（v2 前无 Category 列）需按本版重新分析。
...
#### 2.1 要求源边界（training-provided 判定，v0.2 增补）

能力词来源只认**「任职要求」段**（及等同表述：岗位要求/胜任要求/资格要求）。
「培养方向/培养机制/岗位定位/轮岗安排」段的内容是**岗位提供的成长内容或工作内容，
不是入职要求**：

| 原文语境 | 判定 | 处理 |
|---------|------|------|
| 「培养/轮岗学习/带教/将熟悉/了解…流程」（岗位会教你） | training-provided | category 只能 soft 或 preference，**禁止 hard** |
| 「负责/承接/参与…工作」（工作内容） | 职责描述 | 仅当任职要求段有对应背书（要求你会）→ hard；无背书 → soft |
| 「要求/需/必须具备/熟练掌握」 | 明确要求 | hard（受 §2.3 行为词约束） |

核心判断句：**「岗位会教你/带你做的」不是「要求你会做的」**——判断依据是原文语境动词，
不是内容本身。轮岗定岗信息进 `context.careerPath`，不进 capabilities。

#### 2.2 OR 语义保留（v0.2 增补）

「A或B」「A/B」「A、B等至少一款」「A及以上」「A等相关专业」类**并列选择**表达：

- **提取为一个能力**，取上位概念词（「Inventor或SW等至少一款」→ 概念词「三维建模」；
  「泵、阀等设备」→ 概念词「设备选型」）
- 工具枚举原文写入 `questions`（如「用什么软件建模出图」）——匹配时命中任一替代即满足
- **禁止拆成多个并列 hard 能力**——拆分会把 OR 变 AND，制造假缺口

#### 2.3 行为/意愿词分类（v0.2 增补）

行为/素质/意愿类词（协作、沟通、学习能力、逻辑、适应、意愿、责任心、抗压、自驱、
客户需求转化、行业技术跟进）→ 只能 soft 或 preference，**禁止 hard**。
hard 只放可声明的技能/工具/知识实体词（词表可对齐的能力）。

#### 2.4 门槛覆盖自查（v0.2 增补）

提交 Proposal 前自查：任职要求段有学历/专业/年限表述 → constraints 表**必须存在**
对应维度行（「优先考虑」→ preferred 不产 hard 维度、保留原文 + medium——既有规则；
漏提取 = 门槛维度缺失，匹配度分母错误收缩）。
...
| 培养型 JD（管培生） | work_mode/career_path 提取（轮岗/定岗） | 正常 | 按 §2.1 要求源边界：培养内容 soft/preference，任职要求段才可 hard |
