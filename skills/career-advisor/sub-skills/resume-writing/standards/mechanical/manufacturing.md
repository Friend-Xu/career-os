...
| 锚点类型 | 示例表述 | 验证要求 |
|---------|---------|---------|
| 良率 | "良率从 X% 提升至 Y%" | 有前后数据 + 时间范围 |
| Cpk | "关键尺寸 Cpk 从 X 提升至 Y" | 有控制图/PPAP 依据 |
| 节拍/UPH | "XX 工序节拍从 X 秒优化至 Y 秒 / UPH 提升 X%" | 有节拍测算 |
| 成本 | "国产化替代年度节省 X 万元 / 单件成本降 X%" | 有对比基线 |
| 换型 | "SMED 换型从 X 分钟降至 Y 分钟" | 有前后对比 |
| 异常 | "累计解决制程异常 X+ 项" | 有记录 |
| 标准化 | "编制 SOP X+ 份，覆盖 X 条产线" | 有交付物 |
| 刀具 | "刀具寿命从 X 件提升至 Y 件" | 有寿命记录 |
...
| L3 主导 | 主导、独立完成、推动闭环 | 决策权证据：方案被采纳、改善闭环、PPAP 通过 |
