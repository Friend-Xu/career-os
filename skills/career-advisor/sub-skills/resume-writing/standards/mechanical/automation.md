...
**可得性档位规则（v1.3）**：节拍/OEE/良率等运行数据多为**工程指标档**（自己调试测得的）；营收/成本节省属业务结果档，组织数据不透明时不编。结果未知 → 写确定性终点（交付验收/上线运行）。**无证据数字 = 最严重违规**。
...
|---------|---------|---------|:--:|
| 节拍 | "产线节拍从 X 秒缩短至 Y 秒" | 有前后对比 | 工程指标 |
| OEE | "设备 OEE 从 X% 提升至 Y%（目标 ≥85%）" | 有运行数据 | 工程指标 |
| 交付量 | "独立交付 X 台套非标设备" | 有交付记录 | 交付物 |
| 成本 | "单台设备成本降低 X 万元" | 有对比基线 | 业务结果 |
| 良率 | "设备运行良率从 X% 提升至 Y%" | 有良率数据 | 工程指标 |
| 停机 | "非计划停机时间降低 X%" | 有停机统计 | 工程指标 |
| 项目规模 | "主导 X 万元级自动化产线方案设计与交付" | 有合同/项目范围 | 交付物 |
...
**「负责」警示（v1.3）**：「负责」×3 以上 = HR 跳过信号——只描述职责范围非个人贡献。职责范围 → 条目头/description；bullet 用成果式动词（独立完成/主导/优化/重构）+ 方法 + 量化结果。动词与职级匹配：执行层「协助/参与/完成」、骨干「主导/设计/优化」——层级错配反向扣分。
