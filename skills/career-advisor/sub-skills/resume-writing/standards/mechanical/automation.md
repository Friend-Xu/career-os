...
| Level | 类型 | 示例 | 证据要求 |
|:--:|------|------|---------|
| L1 | 明确量化 | 节拍 25s→18s / OEE 78%→86% | 数值 + 指标名 + 对象 |
| L2 | 范围化/阶段化 | 缩短调试周期 / 提升产线稳定性 / 完成验收闭环 | 有范围或阶段描述 |
| L3 | 事实结果 | 通过客户验收 / 满足节拍目标 / 交付上线 | 仅事实 |

规则：**不得从 Level 3 推导 Level 1**（"通过客户验收" ≠ "良率提升 20%"）；基线无数值且无范围证据时，允许 L2/L3 表述并标注"建议追问量化"。
...
| L3 主导 | 主导、独立完成、推动落地 | 责任证据：方案定夺、跨团队协调、交付责任 |
