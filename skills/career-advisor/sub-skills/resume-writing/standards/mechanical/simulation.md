...
> 修订（2026-08-13，v1.3）：撰写规范调研（writing-norms.md）——量化锚点可得性标注 + 「负责」警示 + 仿真 7 步设计迭代结构
> 置信度总评：High
...
**可得性档位规则（v1.3）**：仿真类指标（应力/温升/压损/对标误差）多为**工程指标档**——自己算出来/测出来的，信息不对称环境下仍可得；业务结果（成本节省）不可得时不编。实验结果非本人执行 → 落交付物档 + 确定性终点（报告交付/评审通过）。**无证据数字 = 最严重违规**。
...
|---------|---------|---------|:--:|
| 应力/变形 | "最大应力降低 X% / 变形控制在 Xmm 内" | 有分析结果 + 判定基准 | 工程指标 |
| 减重 | "拓扑优化减重 X%，通过强度验证" | 有验证闭环 | 工程指标 |
| 对标 | "仿真与试验误差 X% 以内" | 有对标数值 | 工程指标 |
| 温度/温升 | "最高温度降低 XK / 整机温升降至 X℃" | 有仿真或实测 | 工程指标 |
| 热阻 | "接触热阻降低 X%" | 有材料/结构依据 | 工程指标 |
| 压损 | "流道压损降低 X%" | 有前后对比 | 工程指标 |
| 寿命 | "预测疲劳寿命 X 万次，安全系数 X.X" | 有载荷谱依据 | 工程指标 |
| 效率 | "收敛时间缩短 X%" | 有方法依据（参数化/二次开发） | 工程指标 |
| 网格 | "完成百万级网格的 XX 模型划分" | 有模型规模数值 | 交付物 |
...
**「负责」警示（v1.3）**：「负责」×3 以上 = HR 跳过信号。职责范围 → 条目头/description；bullet 用成果式动词（独立完成/主导/优化/重构）+ 方法 + 量化结果。动词与职级匹配：执行层「协助/参与/完成」、骨干「主导/分析/优化」——层级错配反向扣分。
...
- **项目概述写 description**（Entry Contract v0.2）：分析对象与目标一句说清——S 背景不进 bullet

### 仿真经历写法：设计迭代，不是仿真报告（v1.3）

技术主管关心的是**根据仿真结果改了什么、为什么改、改对了没有**——不是网格画得多细。七步结构：

```
分析对象 → 工况定义（载荷/约束/安全系数）→ 分析结果（应力/变形数值）
→ 工程判断 → 结构改进方案（改了什么参数）→ 改进后结果 → 实物测试验证
```

- 只"算过"没有"改过"的仿真经历是减分项（区别于仿真报告——简历不是报告）
- 工程判断必须可追溯：为什么判定不合格（对照什么基准），为什么选这个改进方向
- 改进后结果与改进前对比（工程指标档）；实物验证不可得 → 确定性终点（改进方案通过评审/被设计采纳）
- 每项目 2-4 bullet 按七步截取：工况+结果一条、判断+改进一条、验证一条
